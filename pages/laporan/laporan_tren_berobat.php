...
// --- QUERY UTAMA: MENGHITUNG TREN DIAGNOSIS ---
$query_tren_diagnosis = "
    SELECT 
        diagnosis,
        COUNT(id) AS total_kasus
    FROM 
        berobat /* Diasumsikan nama tabel riwayat berobat adalah 'berobat' */
    WHERE 
        tanggal_berobat BETWEEN '$datetime_awal' AND '$datetime_akhir'
    ...
    LIMIT 5
";
$result_tren = mysqli_query($koneksi, $query_tren_diagnosis);
...
$grand_total = array_sum(array_column($data_tren, 'total_kasus'));

// --- LOGIKA EKSPOR EXCEL (CSV) ---
if (isset($_GET['action']) && $_GET['action'] == 'export_csv') {
    $filename = "Tren_Diagnosis_" . date('Ymd') . ".csv";
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="' . $filename . '"');
    $output = fopen('php://output', 'w');
    
    fputcsv($output, ['Peringkat', 'Diagnosis', 'Total Kasus', 'Persentase']);
    
    $no = 1;
    foreach ($data_tren as $data) {
        $percentage = ($data['total_kasus'] / $grand_total) * 100;
        fputcsv($output, [
            $no++,
            $data['diagnosis'],
            $data['total_kasus'],
    ...
    fputcsv($output, ['', 'TOTAL KASUS', $grand_total, '']);
...
<html lang="en">
<head>
    <title>Cetak Tren Diagnosis</title>
    <style>
...
<body onload="window.print()">
    <div class="header-laporan">
        <h3>LAPORAN TREN DIAGNOSIS TOP 5</h3>
    ...
    <table>
        ...
        <tbody>
            <?php $no = 1; foreach ($data_tren as $data): 
                $percentage = ($data['total_kasus'] / $grand_total) * 100;
            ?>
            <tr>
                <td><?= $no++; ?></td>
...
<html lang="en">
<head>
    <title>Laporan Tren Diagnosis</title>
    ...
    <div id="app">
        ...
        <div id="main">
            ...
            <div class="page-heading">
                <h3>📊 Laporan Tren Diagnosis & Penyakit</h3>
                <p class="text-subtitle text-muted">Analisis 5 diagnosis tertinggi periode **<?= date('d/m/Y', strtotime($tanggal_awal)) ?>** s/d **<?= date('d/m/Y', strtotime($tanggal_akhir)) ?>**.</p>
            </div>
            ...
            <section class="section">
                ...
                <div class="card card-analytics">
                    ...
                    <div class="card-body">
                        <form action="laporan_tren_berobat.php" method="GET" class="row g-3 align-items-end">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-6 text-end">
                                ...
                                <a href="laporan_tren_berobat.php?tgl_awal=<?= $tanggal_awal ?>&tgl_akhir=<?= $tanggal_akhir ?>&action=export_csv" class="btn btn-success me-2">
                                    <i class="bi bi-file-earmark-excel"></i> Export CSV
                                </a>
                                <a href="laporan_tren_berobat.php?tgl_awal=<?= $tanggal_awal ?>&tgl_akhir=<?= $tanggal_akhir ?>&print=true" target="_blank" class="btn btn-outline-danger">
                ...
                <div class="card card-analytics">
                    <div class="card-header bg-info text-white">
                        <h5 class="card-title mb-0"><i class="bi bi-bar-chart-fill me-2"></i> Top 5 Diagnosis Terbanyak</h5>
                    </div>
                    <div class="card-body">
                        <?php if (empty($data_tren)): ?>
                            <div class="alert alert-light-warning text-center">Tidak ada data diagnosis untuk periode ini.</div>
                        <?php else: 
                            $colors = ['bg-danger', 'bg-warning', 'bg-info', 'bg-primary', 'bg-success'];
                            $i = 0;
                        ?>
                            <?php foreach ($data_tren as $data): 
                                $percentage = ($data['total_kasus'] / $grand_total) * 100;
                                $color = $colors[$i % count($colors)];
                            ?>
                                <div class="mb-3">
                                    <div class="progress-label">
                                        <span><?= htmlspecialchars($data['diagnosis']) ?></span>
                            ...
                            <?php $i++; endforeach; ?>
                            <hr>
                            <h5 class="text-end">Total Semua Kasus: <span class="text-primary"><?= number_format($grand_total) ?></span></h5>
                        <?php endif; ?>
                    </div>
                </div>
            </section>
